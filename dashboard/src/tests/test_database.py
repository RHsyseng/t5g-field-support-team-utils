...
class TestDatabaseModels:
    ...
    def test_case_model_creation(self, test_db_session):
        ...
        retrieved_case = test_db_session.query(Case).filter_by(case_number="12345678").first()
    ...
    def test_jira_card_model_creation(self, test_db_session):
        ...
        retrieved_card = test_db_session.query(JiraCard).filter_by(jira_card_id="TEST-123").first()
    ...
    def test_jira_comment_model_creation(self, test_db_session):
        ...
        # Verify relationships
        retrieved_comment = test_db_session.query(JiraComment).filter_by(jira_comment_id="comment-123").first()
...
class TestDatabaseOperations:
    ...
    def test_load_cases_postgres_with_fake_data(self, mock_session, fake_data, test_db_session):
        ...
        for case_number, case_data in cases_data.items():
            if "product_version" not in case_data:
                ...
                case_data["product_version"] = version_match.group(1) if version_match else "1.0"
        ...
        case_81381364 = test_db_session.query(Case).filter_by(case_number="81381364").first()
    ...
    @patch('t5gweb.database.operations.scoped_session')
    def test_load_jira_cards_postgres_with_mock_issue(self, mock_session, test_db_session, mock_jira_issue, fake_data):
        ...
        with patch('t5gweb.database.operations.format_comment', side_effect=lambda x: x.body):
            # Load JIRA card
        ...
        jira_card = test_db_session.query(JiraCard).filter_by(jira_card_id="TEST-123").first()
        ...
        comments = test_db_session.query(JiraComment).filter_by(jira_card_id="TEST-123").all()
        assert len(comments) == 2
    
    # @patch('t5gweb.database.operations.scoped_session')
    # def test_load_jira_cards_postgres_optimized(self, mock_session, test_db_session, mock_jira_issue):
...
class TestDataIntegrity:
    ...
    def test_duplicate_case_handling(self, mock_session, test_db_session, fake_data):
        ...
        if "product_version" not in single_case["81381364"]:
            ...
            version_match = re.search(r'[\s]+([\d\.]+)$', product)
            single_case["81381364"]["product_version"] = version_match.group(1) if version_match else "1.0"
    ...
    def test_foreign_key_constraints(self, test_db_session):
        ...
        try:
            ...
            # If we reach here in SQLite, that's expected since FK constraints may not be enforced
            # But we can still verify the card was created (which shouldn't happen in production with FK constraints)
            orphan_card = test_db_session.query(JiraCard).filter_by(jira_card_id="ORPHAN-123").first()
            assert orphan_card is not None  # In SQLite, this might succeed
            
        except Exception as e:
            # If an exception is raised, that's good - it means FK constraints are working
    ...
    def test_severity_parsing(self, test_db_session):
        ...
        for i, (severity_string, expected_int) in enumerate(test_cases):
            case = Case(
                ...
                created_date=datetime.now(timezone.utc),
                owner="Test",
                severity=int(severity_string[0]) if severity_string and severity_string[0].isdigit() else None,
                account="Test",
                summary="Test",
                status="Open",
                last_update=datetime.now(timezone.utc),
...
class TestPerformanceAndScaling:
    ...
    @patch('t5gweb.database.operations.scoped_session')
    def test_bulk_case_loading_performance(self, mock_session, test_db_session, fake_data):
    ...
    def test_query_efficiency_with_relationships(self, test_db_session):
        ...
        case_with_cards = test_db_session.query(Case).filter_by(case_number="12345678").first()
