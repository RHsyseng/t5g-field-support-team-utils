...
def _process_single_case(case, cases, context, cfg):
    ...
    if assignee and assignee.get("notifieduser", "true") == "true":
        _add_case_watcher(cfg, case, assignee["jira_user"], context["token"])
...
def _handle_old_case(case, context, cfg):
    ...
    if previous_issue:
        logging.warning(f"Updating: {previous_issue.key} rather than creating new card.")
        context["jira_conn"].add_issues_to_sprint(context["sprint"].id, [previous_issue.key])
...
def _add_case_watcher(cfg, case, jira_user, token):
    """Add watcher to the case"""
    logging.warning(f"Adding watcher {jira_user} to case {case}")
    add_watcher_to_case(cfg, case, jira_user, token)
...
def _build_card_info(case, cases, cfg, assignee):
    ...
    full_description = (
        ...
        "\r\n"
        f"This card was created because it had a severity of {cases[case]['severity']}\r\n"
        f"The account for the case is {cases[case]['account']}\r\n"
        f"The case had an internal status of: {cases[case]['status']}\r\n"
        "\r\n*Description:* \r\n\r\n"
        f"{cases[case]['description']}\r\n"
    )
    ...
    card_info = {
        ...
        "description": (
            full_description[:253] + ".." if len(full_description) > 253 else full_description
        ),
    }
...
def _add_card_links(new_card, case, cases, jira_conn):
    ...
    if "bug" in cases[case]:
        logging.warning(f"Adding link to BZ {cases[case]['bug']}")
        jira_conn.add_simple_link(
            new_card.key,
            {
                "url": f"https://bugzilla.redhat.com/show_bug.cgi?id={cases[case]['bug']}",
                "title": f"BZ {cases[case]['bug']}",
            },
        )
