...
class WatcherCase:
    def __init__(self):
        ...
        self.API_URL = os.getenv(
            "REDHAT_API_URL", default="https://api.access.redhat.com/support"
        )
    ...
    def init_parser(self):
        ...
        help_parser.add_argument(
            "command", nargs="?", help="Subcommand to show help for"
        )

        # List subcommand
        list_parser = subparsers.add_parser(
            "list", help="List the given users and support cases as watchers"
        )
        list_parser.add_argument(
            "--users", "-u", nargs="+", help="List of user IDs to check"
        )
        list_parser.add_argument(
            "--cases", "-c", nargs="+", help="List of case IDs to check"
        )
        list_parser.add_argument(
            "-f", "--filename", help="Name of the input file with user and case IDs"
        )

        # Add subcommand
        add_parser = subparsers.add_parser(
            "add", help="Add the given users as watchers to the given support cases"
        )
        add_parser.add_argument(
            "--users", "-u", nargs="+", help="List of user IDs to add as watchers"
        )
        add_parser.add_argument(
            "--cases",
            "-c",
            nargs="+",
            help="List of case IDs to add the users as watchers to",
        )
        add_parser.add_argument(
            "-f", "--filename", help="Name of the input file with user and case IDs"
        )
        ...
        del_parser = subparsers.add_parser(
            "del", help="Del the given users as watchers to the given support cases"
        )
        del_parser.add_argument(
            "--users", "-u", nargs="+", help="List of user IDs to delete as watchers"
        )
        del_parser.add_argument(
            "--cases",
            "-c",
            nargs="+",
            help="List of case IDs to delete the users as watchers to",
        )
        del_parser.add_argument(
            "-f", "--filename", help="Name of the input file with user and case IDs"
        )
    ...
    def update_watchers(self, case_ids, user_ids, action):
        ...
        payload = {"user": [{"ssoUsername": user_id} for user_id in user_ids]}
            ...
            if action == "list":
                ...
                for user_id in user_ids:
                    if user_id in sso_usernames:
                        print(
                            Fore.GREEN
                            + f"User {user_id} is a watcher on case {case_id}"
                        )
                    else:
                        print(
                            Fore.RED
                            + f"User {user_id} is not a watcher on case {case_id}"
                        )
            elif action == "add":
                ...
                if to_remove:
                    ...
                    print(
                        Fore.GREEN
                        + f'Users {", ".join(user_ids)} not watchers on case {case_id}'
                    )
