...
def case_report(api_url, case_tag):
    ...
    closed_cases = {c: d for (c, d) in cases.items() if d['status'] == 'Closed' and
    (today - datetime.datetime.strptime(d['last_update'], '%Y-%m-%dT%H:%M:%SZ').date()).days < age}

    opened_cases = {c: d for (c, d) in cases.items() if d['status'] != 'Closed' and
    (today - datetime.datetime.strptime(d['createdate'], '%Y-%m-%dT%H:%M:%SZ').date()).days < age}
...
def main():
    ...
    print("searching {} for newly opened and closed cases ({})".format(api_url, case_tag))
